    ...
    Promise.all([
      ...
      if (blogRes.success) {
        ...
        setForm({
          ...
          tagIds: blog.tags ? blog.tags.map((t: { tagId: string }) => t.tagId) : [],
        });
      }
        ...
        <div className="flex items-center gap-3">
          {form.status === "PUBLISHED" && (
            <Link
              href={`/blog/${form.slug}`}
              target="_blank"
              className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-black/60 border border-black/10 rounded-xl hover:bg-black/[0.02]"
            >
